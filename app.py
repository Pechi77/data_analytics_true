...
def build_sample_db():
    ...
    with app.app_context():
        ...
        mongo = PyMongo(app)
        framework = mongo.db.brands
        for user in framework.find(): 
            user_datastore.create_user(
                first_name=user['brand_name'],
                last_name=user['brand_name'],
                email=user['brand_name'],
                password=encrypt_password(user['brand_name']),
                roles=["Role" ]
            )
        db.session.commit()
    return
